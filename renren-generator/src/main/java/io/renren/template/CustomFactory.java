...
public class CustomFactory {
    private Map<String,ExecTemplate> objectMap = new HashMap<>();
    ...
    public List<ExecTemplate> getTemplatesList(){
        ArrayList<ExecTemplate> objects = new ArrayList<>();
        objectMap.forEach((s, o) -> {
            objects.add(o);
        });
        return objects;
    ...
    public CustomFactory(String path) {
        try {
            // 获取包名下所有类
            Set<Class<?>> classes = getClasses(path);
            for(Class c:classes){
                if(c.isAnnotationPresent(FactoryClass.class)){
                    objectMap.put(c.getSimpleName().toLowerCase(),(ExecTemplate)c.newInstance());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Map<String, ExecTemplate> getObjectMap() {
        return objectMap;
    }
