 ...
 * @author ghzj
 */
public class CustomTemplateFactory {

    private final List<ExecTemplate> objectList = new ArrayList<>();
    private final Map<String, String> MAP = new HashMap<>();

    private final String templatePath;

    public List<String> gettemplates(){
        return objectList.stream().map(ExecTemplate::getName).collect(Collectors.toList());
    }

    /**
     * 获取文件名
     */
    public String getFileName(String template, Map<String, Object> map) {
        String packagePath = "main" + File.separator + "java" + File.separator;
        String packageName = (String)map.get("packagePath");
        ...
        if (!CollectionUtils.isEmpty(objectList)){
            for (ExecTemplate execTemplate : objectList) {
    ...
    public CustomTemplateFactory(String path,String templatePath) {
        try {
            // 获取包名下所有类
            Set<Class<?>> classes = getClasses(path);
            for(Class c:classes){
                if(c.isAnnotationPresent(TemplateAnnotation.class)){
                    ExecTemplate execTemplate = (ExecTemplate) c.newInstance();
                    String[] split = execTemplate.getTemplateName().split(":");
                    MAP.put(split[0],split[1]);
                    String[] split1 = execTemplate.getTemplatePathName().split(":");
                    MAP.put(split1[0],split1[1]);
                    objectList.add(execTemplate);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
